...
static const sd_bus_vtable node_vtable[] = { SD_BUS_VTABLE_START(0),
					     SD_BUS_VTABLE_END };
...
static void job_isolate_all_request_job_done(sd_bus_message *m,
					     const char *result,
					     void *userdata) {
...
static int job_isolate_all_request_cb(sd_bus_message *m, void *userdata,
	...
	if (sd_bus_message_is_method_error(m, NULL)) {
		...
		r = sd_bus_message_read(request->reply, "o",
					&request->job_object_path);
		if (r < 0) {
			fprintf(stderr,
				"Failed to parse isolate response: %s\n",
				strerror(-r));
			request->result = JOB_FAILED;
			isolate_all->n_outstanding_requests--;
		} else {
...
static int job_isolate_all(Job *job) {
	...
	LIST_FOREACH(nodes, node, orch->nodes) {
		...
		r = sd_bus_message_new_method_call(
			node->peer, &request->request, NODE_BUS_NAME,
			NODE_PEER_OBJECT_PATH, NODE_PEER_IFACE, "Isolate");
		if (r >= 0)
			r = sd_bus_message_append(request->request, "s",
						  isolate_all->target);
		if (r >= 0)
			r = sd_bus_call_async(node->peer,
					      &request->request_slot,
					      request->request,
					      job_isolate_all_request_cb,
					      request, DEFAULT_DBUS_TIMEOUT);
		if (r < 0) {
...
static int method_orchestrator_isolate_all(sd_bus_message *m, void *userdata,
					   sd_bus_error *ret_error) {
	...
	r = manager_queue_job(manager, JOB_ISOLATE_ALL, sizeof(IsolateAllJob),
			      m, job_isolate_all, cancel_isolate_all,
			      job_isolate_all_destroy, &job);
	if (r < 0)
		return sd_bus_reply_method_errnof(m, -r,
						  "Failed to create job: %m");
...
static const sd_bus_vtable orchestrator_vtable[] = {
	...
	SD_BUS_METHOD("IsolateAll", "s", "o", method_orchestrator_isolate_all,
		      0),
	SD_BUS_SIGNAL_WITH_NAMES("JobNew", "uo",
				 SD_BUS_PARAM(id) SD_BUS_PARAM(job), 0),
	SD_BUS_SIGNAL_WITH_NAMES(
		"JobRemoved", "uos",
		SD_BUS_PARAM(id) SD_BUS_PARAM(job) SD_BUS_PARAM(result), 0),
	SD_BUS_VTABLE_END
};
...
static int node_disconnected(sd_bus_message *message, void *userdata,
			     sd_bus_error *error) {
	Node *node = userdata;
...
static int method_peer_hello(sd_bus_message *m, void *userdata,
			     sd_bus_error *ret_error) {
...
static const sd_bus_vtable peer_bus_vtable[] = {
	SD_BUS_VTABLE_START(0),
	SD_BUS_METHOD("Hello", "", "s", method_peer_hello,
		      SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_VTABLE_END
};

static int all_node_messages_handler(sd_bus_message *m, void *userdata,
				     sd_bus_error *ret_error) {
	Node *node = userdata;
	if (node->name)
		printf("Incomming message from node '%s' (fd %d): path: %s, iface: %s, member: %s, signature: '%s'\n",
		       node->name, sd_bus_get_fd(node->peer),
		...
		printf("Incomming message from node fd %d: path: %s, iface: %s, member: %s, signature: '%s'\n",
		       sd_bus_get_fd(node->peer), sd_bus_message_get_path(m),
		       sd_bus_message_get_interface(m),
		       sd_bus_message_get_member(m),
		       sd_bus_message_get_signature(m, true));
	return 0;
...
static int accept_handler(sd_event_source *s, int fd, uint32_t revents,
	...
	r = sd_bus_set_trusted(
		bus,
		true); /* we trust everything from the node, there is only one peer anyway */
	if (r < 0) {
	...
	if (r < 0) {
		fprintf(stderr,
			"Failed to enable server support for new connection bus: %s\n",
			strerror(-r));
		return 0;
	}
	...
	if (r < 0) {
		fprintf(stderr,
			"Failed to enable credentials for new connection: %s\n",
			strerror(-r));
		return 0;
	}
	...
	r = sd_bus_start(bus);
	if (r < 0) {
		fprintf(stderr, "Failed to start new connection bus: %s",
			strerror(-r));
		return 0;
	}
	...
	r = sd_bus_add_object_vtable(node->peer, NULL, "/org/freedesktop/DBus",
				     "org.freedesktop.DBus", peer_bus_vtable,
				     node);
	if (r < 0) {
		fprintf(stderr, "Failed to add peer bus vtable: %s\n",
			strerror(-r));
	...
	r = sd_bus_match_signal(node->peer, NULL, NULL, NODE_PEER_OBJECT_PATH,
	...
	r = sd_bus_add_object_vtable(node->peer, NULL, ORCHESTRATOR_OBJECT_PATH,
				     ORCHESTRATOR_PEER_IFACE,
				     peer_orchestrator_vtable, node);
	if (r < 0) {
		fprintf(stderr, "Failed to add peer bus vtable: %s\n",
			strerror(-r));
		return EXIT_FAILURE;
	...
	r = sd_bus_match_signal_async(
		node->peer, NULL, "org.freedesktop.DBus.Local",
		"/org/freedesktop/DBus/Local", "org.freedesktop.DBus.Local",
		"Disconnected", node_disconnected, NULL, node);
	if (r < 0) {
		fprintf(stderr,
	...
	if (DEBUG_DBUS_MESSAGES)
		sd_bus_add_filter(node->peer, NULL, all_node_messages_handler,
				  node);
...
static int all_bus_messages_handler(sd_bus_message *m, void *userdata,
				    sd_bus_error *ret_error) {
...
int main(int argc, char *argv[]) {
	...
	orchestrator.manager.job_path_prefix =
		ORCHESTRATOR_JOBS_OBJECT_PATH_PREFIX;
	orchestrator.manager.manager_path = ORCHESTRATOR_OBJECT_PATH;
	orchestrator.manager.manager_iface = ORCHESTRATOR_IFACE;

	r = sd_bus_add_object_vtable(bus, &slot, ORCHESTRATOR_OBJECT_PATH,
				     ORCHESTRATOR_IFACE, orchestrator_vtable,
				     &orchestrator);
	if (r < 0) {
