...
static int get_options(
                Command *command, bool *ret_should_monitor, long *ret_initial_wait, long *ret_switch_timeout) {
        ...
        const char *switch_timeout = command_get_option(command, ARG_SWITCH_CTRL_TIMEOUT_SHORT);
        long parsed_switch_timeout = 0;
        if (switch_timeout != NULL && !streq(switch_timeout, "") &&
            !parse_long(switch_timeout, &parsed_switch_timeout)) {
                fprintf(stderr, "Failed to parse --%s=%s\n", ARG_SWITCH_CTRL_TIMEOUT, switch_timeout);
                return -EINVAL;
        }

        *ret_switch_timeout = parsed_switch_timeout;
...
int method_is_online_agent(Command *command, UNUSED void *userdata) {
        ...
        r = get_options(command, &should_monitor, &initial_wait, &switch_timeout);
        if (r < 0) {
                return r;
        }
